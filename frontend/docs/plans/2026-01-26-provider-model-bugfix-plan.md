...
## Task 1: Add Debug Logging for Data Flow Tracing

**Files:**
- Modify: `src/components/Agents/AgentForm.tsx:173-194`
- Modify: `src/components/Agents/CreateAgentDialog.tsx:83-110`

**Purpose:** Before fixing bugs, we need to see exactly what values flow through the system. This task adds temporary console.log statements that will be removed after debugging.

**Step 1: Add logging to AgentForm onChange effect**

In `AgentForm.tsx`, find the `useEffect` that calls `onChange` (around line 173) and add logging:
...
**Step 2: Add logging to CreateAgentDialog handleSubmit**

In `CreateAgentDialog.tsx`, find `handleSubmit` (around line 83) and add logging before mutation:
...
4. Check browser console for the 3 log messages

Expected console output:
```
[AgentForm] onChange called with: {selectedProviderId: "<uuid>", derivedProviderType: "openai", ...}
[CreateAgentDialog] formData received: {provider_type: "openai", user_provider: "<uuid>", ...}
[CreateAgentDialog] API payload: {provider_type: "openai", user_provider: "<uuid>", ...}
```

**Step 4: Commit debug logging**

```bash
git add src/components/Agents/AgentForm.tsx src/components/Agents/CreateAgentDialog.tsx
git commit -m "chore: add debug logging for provider/model data flow"
```
...
**Purpose:** Using the debug logging from Task 1, identify where `user_provider` is being lost and fix it.
...
3. Fill in required fields (name, etc.)
4. Click "Create Agent"
5. Check console output

**Step 2: Identify the failure point**

Check console logs for where the value disappears:

| If this log shows null/undefined | The bug is in |
|----------------------------------|---------------|
| `[AgentForm] onChange: selectedProviderId` | `AgentForm.tsx` - state not updating |
| `[CreateAgentDialog] formData.user_provider` | `AgentForm.tsx` - not passing to onChange |
| `[CreateAgentDialog] payload.user_provider` | `CreateAgentDialog.tsx` - payload construction |
| API request body (Network tab) | `agentService.ts` - not sending field |

**Step 3: Apply the fix based on findings**

**If the issue is in AgentForm state:**
