          ...
          <CardContent className="space-y-4">
            ...
            <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              ...
              <div>
                ...
                <dd>
                  {workspace.lastTransitionAt?.toLocaleString() ??
                    "Not recorded"}
                </dd>
              </div>
              <div>
            ...
            workspace.accessLevel !== "manage" ? (
              <div className="rounded-lg border bg-muted/20 p-4 text-sm text-muted-foreground">
                This workspace is available through project membership. You can
                use the capabilities exposed by the backend, while
                runtime-destructive actions remain owner-scoped in the current
                policy pass.
              </div>
            ) : null}
              ...
              <div className="space-y-3 rounded-xl border bg-muted/20 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Runtime surfaces inferred from bootstrap profiles and
                      kennel discovery.
                    </div>
                  </div>
                ...
                {agentRuntimeServices.length > 0 ? (
                  <div className="rounded-lg border bg-background/70 p-3">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      ...
                      processes. They may not publish a browser URL, but they
                      still participate in readiness and future room
                      connectivity.
                    </div>
                  </div>
                ) : null}
                ...
                {webServices.length === 0 && agentRuntimeServices.length > 0 ? (
                  <div className="text-xs text-muted-foreground">
                    ...
                    oriented around agent runtime availability rather than
                    browser traffic.
                  </div>
                ) : null}
              </div>
            ...
            <div className="space-y-3 rounded-xl border bg-muted/20 p-4">
                ...
                <div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-64">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="agent_runtime">
                        Agent Runtime
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              {!workspace.canDiscoverServices ? (
                <div className="rounded-lg border bg-background/70 p-3 text-sm text-muted-foreground">
                  Platform-service grants become available when this workspace
                  is in a service-discoverable state and your current access
                  level includes discovery.
                </div>
              ) : null}
              {workspaceUnavailableForRuntimeAccess ? (
                <div className="rounded-lg border bg-background/70 p-3 text-sm text-muted-foreground">
                  This workspace is no longer in a runtime-access state.
                  Existing projected access should be treated as historical
                  context, and fresh runtime access should be requested only
                  after the workspace returns to a ready state or a replacement
              ...
              {issuePlatformAccess.data ? (
                <div className="space-y-3">
                  <div className="rounded-lg border bg-background/70 p-3 text-sm">
                    <div className="font-medium">
                      Current grant for{" "}
                      {issuePlatformAccess.data.consumerKind.replaceAll(
                        "_",
                        " ",
                      )}
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      Issued{" "}
                      {issuePlatformAccess.data.issuedAt.toLocaleString()}
              ...
              <div className="space-y-3 rounded-xl border bg-muted/20 p-4">
                ...
                <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <dd>
                      {workspace.flavourHealth.latestRebuildJobId ??
                        "Not recorded"}
                    </dd>
                  </div>
                  <div>
