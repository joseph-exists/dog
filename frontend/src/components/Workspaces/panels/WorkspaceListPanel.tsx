...
function accessLevelLabel(
  value: WorkspaceListItemViewModel["accessLevel"],
): string {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Track provisioning, open ready environments, and keep an eye on
          lifecycle state.
        </CardDescription>
      </CardHeader>
        ...
        {!isLoading && !error && workspaces.length === 0 ? (
          <div className="rounded-xl border border-dashed p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Provision a workspace to start validating the kennel integration
              flow.
            </p>
          </div>
        ) : null}
        ...
        {!isLoading && !error && workspaces.length > 0 ? (
          <div className="space-y-3">
            {workspaces.map((workspace) => {
              const agentRuntime = workspace.services.find(
                (service) => service.kind === "agent_runtime",
              )
              const primaryWebService = workspace.services.find(
                (service) => service.url,
              )
              return (
                <div
                ...
                >
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Visibility: {workspace.visibility} · Terminal:{" "}
                      {workspace.terminalStatus}
                    </div>
                    {workspace.isProjectWorkspace &&
                    workspace.accessLevel !== "manage" ? (
                      <div className="text-sm text-muted-foreground">
                        This workspace is shared through project access. Runtime
                        management remains owner-scoped in the current slice.
                      </div>
                    ) : null}
                    {agentRuntime ? (
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        ...
                        <span
                          className={cn(
                            ...
                            serviceStatusClass(agentRuntime.status),
                          )}
                        >
                          {agentRuntime.status}
                        </span>
                    ...
                    workspace.services.length > 0 ? (
                      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {workspace.connectivitySummary?.terminalReady ? (
                      ...
                      <div className="space-y-2 rounded-lg border bg-muted/20 p-3">
                        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                          {workspace.bootstrapIntent?.repoSource ? (
                            <span>
                              {workspace.bootstrapIntent.repoSource.label}:{" "}
                              {workspace.bootstrapIntent.repoSource.detail}
                            </span>
                          ) : (
                            ...
                            {workspace.bootstrapProgress.stepCount ? (
                              <div className="space-y-1">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {workspace.bootstrapProgress.completedSteps ??
                                    0}
                      ...
                      <div className="space-y-2 rounded-lg border bg-muted/20 p-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {workspace.services.slice(0, 3).map((service) => (
                            ...
                            >
                              {service.label} · {serviceKindLabel(service.kind)}{" "}
                              · {formatServiceLabel(service.status)}
                            </span>
                          ))}
                        ...
                        {primaryWebService?.url ? (
                          ...
                          <div className="text-sm text-muted-foreground">
                            Agent runtime readiness is tracked through discovery
                            even when no browser-facing endpoint is published.
                          </div>
                        ) : null}
                      </div>
                    ) : null}
