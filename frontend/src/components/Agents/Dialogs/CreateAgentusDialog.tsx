...
import AgentForm, { type AgentFormData } from "../Forms/AgentusFormulaicus"
...
export default function CreateAgentusDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (created) => {
      showSuccessToast(`Agentus "${created.name}" invoked.`)
      handleOpenChange(false)
      onSuccess?.(created)
    },
    onError: (err: ApiError) => {
      const detail = (err.body as { detail?: string })?.detail
      showErrorToast(detail || "you shall suffer for this indignity")
    },
    onSettled: () => {
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            the one who makes is made through the making
          </DialogTitle>
          <DialogDescription>
            the raven quoth and kept quothing
          </DialogDescription>
        </DialogHeader>
