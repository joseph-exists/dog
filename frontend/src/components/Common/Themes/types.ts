...
export function isStoryTypeKey(
  s: string | null | undefined,
): s is StoryTypeKey {
  return (
    s === "review" ||
...
export interface PresentationTokens {
  ...
  // story-specific accent — already defined in index.css
  "--story-accent"?: string
  "--story-accent-foreground"?: string

  // Extended: card-level visual properties (not in shadcn defaults)
  "--story-card-shadow"?: string
  "--story-card-radius"?: string
  "--story-accent-position"?: "top" | "bottom" | "left" | "none"
  "--story-accent-width"?: string
}
