  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Builder-side preflight checks for story/runtime dependencies and composition constraints.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {issues.length === 0 ? (
          <p className="text-sm text-muted-foreground">No semantic issues detected.</p>
        ) : (
          issues.map((issue, index) => (
            ...
            >
              ...
              {issue.path && (
                <div className="text-xs text-muted-foreground mt-1">Path: {issue.path}</div>
              )}
            </div>
          ))
        )}
