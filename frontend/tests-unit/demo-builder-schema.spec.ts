...
test.describe("demoBuilderSchema v2 field specs", () => {
  ...
  test("panel schema map covers all active panel kinds and includes theme field metadata", async () => {
    expect(Object.keys(BUILDER_PANEL_KIND_SCHEMAS).sort()).toEqual(
      [...ACTIVE_BUILDER_PANEL_KINDS].sort(),
    )
    for (const kind of ACTIVE_BUILDER_PANEL_KINDS) {
      const schema = getBuilderPanelKindSchema(kind)
      expect(schema.kind).toBe(kind)
      expect(
        schema.fieldSpecs.some((field) => field.key === "theme_id"),
      ).toBeTruthy()
    }
  })

  test("block schema map covers all active block types and includes theme field metadata", async () => {
    expect(Object.keys(BUILDER_BLOCK_TYPE_SCHEMAS).sort()).toEqual(
      [...ACTIVE_BUILDER_BLOCK_TYPES].sort(),
    )
...
test.describe("demoBuilderSchema semantic validation", () => {
  ...
  test("flags multiple page viewport panels", async () => {
    ...
    const issues = validateCompositionSemantics(composition)
    expect(
      issues.some((issue) => issue.code === "too_many_page_viewports"),
    ).toBeTruthy()
  })
  ...
  test("flags missing content payload for content-capable blocks", async () => {
    ...
    const issues = validateCompositionSemantics(composition)
    expect(
      issues.some((issue) => issue.code === "content_payload_missing"),
    ).toBeTruthy()
  })
  ...
  test("composition C template applies explicit visibility permutations", async () => {
    ...
    const visibilities = (composition.blocks ?? []).map(
  ...
  test("composition D template includes stylized chat and preloaded participant metadata", async () => {
    ...
    const chatPanel = (composition.panels ?? []).find(
      (panel) => panel.kind === "chat",
    )
    ...
    expect(chatPanel?.presentation_json).toEqual(
  ...
  test("composition E template uses tabs layout with content + git/file coverage", async () => {
    ...
    expect(panelKinds).toEqual(
      expect.arrayContaining(["content", "chat", "debug"]),
    )
    expect(blockTypes).toEqual(
      expect.arrayContaining(["gitView", "fileExplorer", "context", "content"]),
    )
    expect(
      validateCompositionSemantics(composition).every(
        (issue) => issue.severity !== "error",
  ...
  test("composition F template covers all active panel kinds and block types for passthrough auditing", async () => {
    ...
    const panelKinds = new Set(
      (composition.panels ?? []).map((panel) => panel.kind),
    )
    const blockTypes = new Set(
      (composition.blocks ?? []).map((block) => block.type),
    )
  ...
  test("composition G template provides full-surface UX style matrix coverage", async () => {
    ...
    const metadata = composition.metadata_json as Record<string, unknown>
    const presentation = composition.presentation_json as Record<
      string,
      unknown
    >
...
test.describe("demoBuilderSchema template checklist status", () => {
  test("tracks story_id as pending/resolved from composition metadata", async () => {
    ...
    expect(
      pendingStatus.items.find((item) => item.id === "story_id")?.resolved,
    ).toBeFalsy()
    ...
    expect(
      resolvedStatus.items.find((item) => item.id === "story_id")?.resolved,
    ).toBeTruthy()
  })
  ...
  test("requires confirmation for runtime/persona/chat assumptions", async () => {
    ...
    expect(
      statusWithoutConfirm.items.find((item) => item.id === "runtime_policy")
        ?.resolved,
    ).toBeFalsy()
    expect(
      statusWithoutConfirm.items.find((item) => item.id === "persona_policy")
        ?.resolved,
    ).toBeFalsy()
    ...
    expect(
      statusWithConfirm.items.find((item) => item.id === "runtime_policy")
        ?.resolved,
    ).toBeTruthy()
    expect(
      statusWithConfirm.items.find((item) => item.id === "persona_policy")
        ?.resolved,
    ).toBeTruthy()
  })
  ...
  test("fixed_user_persona_id item only blocks when fixed persona policy is selected", async () => {
    ...
    expect(
      status.items.find((item) => item.id === "fixed_user_persona_id")
        ?.resolved,
    ).toBeTruthy()
  ...
  test("template schema exposes checklist metadata", async () => {
    const schema = getBuilderCompositionTemplateSchema(
      "composition_c_visibility_semantics",
    )
    expect(schema.requiredAssumptions).toEqual(
      expect.arrayContaining(["story_id", "runtime_policy"]),
    )
