...
// let's leave this here for now.  I'm interested in monitoring its performance, even though it's not strictly best practice.  we do have validation on the server - but this feels like it's not expensive, and would be faster to process locally (in browser) than send the call to check over the wire.
