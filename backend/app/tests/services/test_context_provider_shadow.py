...
async def test_build_room_context_includes_shadow_items_when_context_store_provided(
    ...
    room_snapshot = {"entity_type": "room", "room": {"room_id": str(room.room_id)}}
    room_shadow_repo = ShadowRepo(
        owner_id=user.id,
        entity_type="room",
        entity_id=room.room_id,
        forgejo_repo_name=f"room-{str(room.room_id)[:8]}",
        forgejo_repo_id=None,
    )
    async_session.add(room_shadow_repo)
    await async_session.commit()
    await async_session.refresh(room_shadow_repo)
    ...
    async_session.add(room_shadow_version)
    await async_session.commit()
