"""Tests for Timeline navigation (undo/jump)."""
...
from app.models import UserNodeChoice, ProgressSnapshot
...
def test_undo_derives_state_from_events_optimized(
    ...
    # Make 2 choices (A → B)
    for _ in range(2):
        resp = client.get(f"/api/v1/user-personas/{persona_id}/stories/{story.id}/current-node", headers=normal_user_token_headers)
        choice_id = resp.json()["available_choices"][0]["id"]
        client.post(f"/api/v1/user-personas/{persona_id}/stories/{story.id}/choices/{choice_id}", headers=normal_user_token_headers)
...
def test_jump_uses_optimized_replay(
    ...
    for _ in range(4):
        resp = client.get(f"/api/v1/user-personas/{persona_id}/stories/{story.id}/current-node", headers=normal_user_token_headers)
        choice_id = resp.json()["available_choices"][0]["id"]
        resp = client.post(f"/api/v1/user-personas/{persona_id}/stories/{story.id}/choices/{choice_id}", headers=normal_user_token_headers)
...
def test_undo_publishes_realtime_event(
    ...
    # TODO: this mock won't work as is, and we already have a full redis setup in tests
