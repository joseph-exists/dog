#!/usr/bin/env python3
"""
Test Agent Integration - Websockets and Quacksworth
...
Results saved to: test_results_agent_integration_new.json
"""
import argparse
import asyncio
import json
...
from auth_helper import AuthenticationError, get_access_token, get_authenticated_session
...
OUTPUT_FILE = "test_results_agent_integration_new.json"
    ...
    for i in range(max_wait):
        ...
        if debug:
            ...
            if messages:
                ...
                print(
                    f"     [Poll {i+1}] Latest: "
                    f"{latest.get('sender_type')} - {latest.get('content', '')[:50]}..."
                )
        ...
        if current_count > initial_message_count:
            ...
            if debug:
                print(
                    f"     [Poll {i+1}] New message found but it's from "
                    f"{latest.get('sender_type')}, not agent"
                )
...
async def _ws_send_and_wait_for_event(
    room_id: str,
    token: str,
    content: str,
    last_sequence: int = 0,
    ...
    """Send a message via WebSocket and wait for room_message.user event."""
    if not websockets:
        print("  ⚠️  websockets library not available; skipping WS test")
        return None

    ws_base = BASE_URL.replace("http://", "ws://").replace("https://", "wss://")
    ws_url = f"{ws_base}/ws/rooms/{room_id}?token={token}"

    async with websockets.connect(ws_url) as websocket:
        await websocket.send(
            json.dumps({"type": "session.create", "last_sequence": last_sequence})
        )
        ...
        while time.time() < deadline:
            ...
            if message.get("type") == "session.created":
                session_ready = True
                await websocket.send(
                    json.dumps({"type": "message.send", "content": content})
                )
                continue

            if (
                message.get("type") == "event"
                and message.get("event_type") == "room_message.user"
            ):
                event_payload = message
                break

        if not session_ready:
            print("  ❌ WebSocket session handshake failed")
            return None

        if not event_payload:
            print("  ❌ WebSocket event not received within timeout")
            return None

        return event_payload


def ws_send_and_validate(
    room_id: str,
    content: str,
    last_sequence: int = 0,
    timeout: int = 10,
) -> dict[str, Any] | None:
    """Sync wrapper for WebSocket send + event validation."""
    token = get_access_token()
    return asyncio.run(
        _ws_send_and_wait_for_event(
            room_id=room_id,
            token=token,
            content=content,
...
Examples:
  ...
  python test_agent_integration.py --output my_results.json
    ...
    results = TestResults()
    total_steps = 12 if not args.no_story else 11
    current_step = 0
    ...
    try:
        ...
        # Step 7: WebSocket Message Send + Receive
        current_step += 1
        print_step(current_step, total_steps, "WebSocket Message Send + Receive")

        ws_message_content = "Hello from websocket test"
        ws_event = ws_send_and_validate(
            room_id=room_id,
            content=ws_message_content,
            last_sequence=0,
            timeout=10,
        )

        if not ws_event:
            results.add_error("websocket", "Failed to receive room_message.user event")
            raise Exception("WebSocket event not received")

        if ws_event.get("payload", {}).get("content") != ws_message_content:
            results.add_error("websocket", "WebSocket payload mismatch")
            raise Exception("WebSocket payload mismatch")

        ws_messages = get_room_messages(session, room_id)
        if not any(m.get("content") == ws_message_content for m in ws_messages):
            results.add_error("websocket", "WebSocket message not persisted")
            raise Exception("WebSocket message not persisted")

        results.add_message("user", ws_message_content, "user", None)
        print("  ✅ WebSocket message delivered and persisted")
        ...
        # Step 10: Test Context Awareness - Story Context
        current_step += 1
        ...
        # Step 11: Test Context Awareness - Message History
        current_step += 1
