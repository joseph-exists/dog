#!/usr/bin/env python3
"""
Forge authentication Helper for Test Scripts

This module provides authentication functionality for forge interactions by:
1. Reading forge credentials from .env file
2. Logging into the API and obtaining access tokens
3. Providing authenticated session objects for other forge operations
...
Usage:
    ...
    response = session.get("http://localhost:3000/api/users/me")
...
Requirements:
    - .env file in backend/ directory with FORGE_ADMIN_TEST_USER_EMAIL and FORGE_ADMIN_TEST_USER_PASSWORD
    - Forge backend server running on localhost:8000
    - Forge Admin test user account created in forge database
"""
