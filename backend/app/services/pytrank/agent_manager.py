...
from pydantic_ai import (
    ...
    exceptions,
    models,
    #  we need to see what breaks when we comment this out.
)
...
# /home/josep/dog/backend/.venv/lib/python3.12/site-packages/pydantic_ai/toolsets/_dynamic.py
...
class Agent(AbstractAgent[AgentDepsT, OutputDataT]):
    ...
    _instructions: list[str | SystemPromptFunc[AgentDepsT]] = dataclasses.field(
        repr=False
    _system_prompt_functions: list[_system_prompt.SystemPromptRunner[AgentDepsT]] = (
        dataclasses.field(repr=False)
    )
    _system_prompt_dynamic_functions: dict[
        str, _system_prompt.SystemPromptRunner[AgentDepsT]
    ] = dataclasses.field(repr=False)
    ...
    _prepare_output_tools: ToolsPrepareFunc[AgentDepsT] | None = dataclasses.field(
        repr=False
    )
    ...
    _event_stream_handler: EventStreamHandler[AgentDepsT] | None = dataclasses.field(
        repr=False
    )
        ...
        **_deprecated_kwargs: Any,
        ...
        if mcp_servers := _deprecated_kwargs.pop("mcp_servers", None):
            if toolsets is not None:  # pragma: no cover
                raise TypeError(
                    "`mcp_servers` and `toolsets` cannot be set at the same time."
                )
            warnings.warn(
        ...
        self._max_result_retries = (
            output_retries if output_retries is not None else retries
        )
        ...
        self._override_name: ContextVar[_utils.Option[str]] = ContextVar(
            "_override_name", default=None
        )
        self._override_deps: ContextVar[_utils.Option[AgentDepsT]] = ContextVar(
            "_override_deps", default=None
        )
        self._override_model: ContextVar[_utils.Option[models.Model]] = ContextVar(
            "_override_model", default=None
        )
        self._override_toolsets: ContextVar[
            _utils.Option[Sequence[AbstractToolset[AgentDepsT]]]
        ] = ContextVar("_override_toolsets", default=None)
        self._override_tools: ContextVar[
            _utils.Option[Sequence[Tool[AgentDepsT] | ToolFuncEither[AgentDepsT, ...]]]
        ] = ContextVar("_override_tools", default=None)
        self._override_instructions: ContextVar[
            _utils.Option[list[str | SystemPromptFunc[AgentDepsT]]]
    ...
    async def iter(
        ...
        *,
        ...
        toolset = self._get_toolset(
            output_toolset=output_toolset, additional_toolsets=toolsets
        )
        ...
        instrumentation_names = InstrumentationNames.for_version(
            instrumentation_settings.version
        ...
        try:
            async with graph.iter(
                ...
                async with toolset:
                    ...
                    ) is not None and run_span.is_recording():
                        if (
                            ...
                            run_span.set_attribute(
                                ...
                                (
                                    ...
                                    if isinstance(final_result.output, str)
                                        ...
                                        InstrumentedModel.serialize_any(
                                            final_result.output
                                        )
                                    )
                                ),
            ...
            attrs = {
                "all_messages_events": json.dumps(
                    [
                        ...
                        for e in settings.messages_to_otel_events(message_history)
                    ]
                )
            }
        else:
            ...
            "logfire.json_schema": json.dumps(
                {
                    ...
                    "properties": {
                        **{
                            k: {"type": "array"} if isinstance(v, str) else {}
    ...
    def override(
        ...
        *,
        ...
        if _utils.is_set(model):
            model_token = self._override_model.set(
                _utils.Some(models.infer_model(model))
            )
        else:
            model_token = None
        ...
        if _utils.is_set(instructions):
            ...
            instructions_token = self._override_instructions.set(
                _utils.Some(normalized_instructions)
            )
        else:
    ...
    @overload
    def instructions(
        self, func: Callable[[], Awaitable[str]], /
        ...
        *,
        ...
        if func is None:
            ...
            self._system_prompt_functions.append(
                _system_prompt.SystemPromptRunner[AgentDepsT](func, dynamic=dynamic)
            )
            return func
    ...
    @overload
    def tool(
        self, func: ToolFuncContext[AgentDepsT, ToolParams], /
    ...
    @overload
    def tool_plain(
        self, func: ToolFuncPlain[ToolParams], /
    ...
    def toolset(
        ...
        *,
        ...
        def toolset_decorator(
            func_: ToolsetFunc[AgentDepsT],
        ) -> ToolsetFunc[AgentDepsT]:
            self._dynamic_toolsets.append(
                DynamicToolset(func_, per_run_step=per_run_step)
            )
            return func_
    ...
    def _get_instructions(
        ...
        if override_instructions:
            ...
            if additional_instructions is not None:
                instructions.extend(
                    self._normalize_instructions(additional_instructions)
                )
        ...
        for instruction in instructions:
            if isinstance(instruction, str):
                ...
                functions.append(
                    _system_prompt.SystemPromptRunner[AgentDepsT](instruction)
                )
    ...
    def _get_toolset(
        self,
        output_toolset: AbstractToolset[AgentDepsT]
        | None
        | _utils.Unset = _utils.UNSET,
        ...
        output_toolset = (
            output_toolset if _utils.is_set(output_toolset) else self._output_toolset
        )
        if output_toolset is not None:
            if self._prepare_output_tools:
                output_toolset = PreparedToolset(
                    output_toolset, self._prepare_output_tools
                )
    ...
    ) -> _output.OutputSchema[OutputDataT]: ...
    ...
    def _prepare_output_schema(
        self, output_type: OutputSpec[Any] | None
    ) -> _output.OutputSchema[Any]:
    ...
    def set_mcp_sampling_model(
        ...
        try:
            sampling_model = (
                models.infer_model(model) if model else self._get_model(None)
            )
        except exceptions.UserError as e:
            raise exceptions.UserError(
                "No sampling model provided and no model set on the agent."
            ) from e
