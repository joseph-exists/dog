...
def _config_debug_summary(config: UserAgentConfig | None, label: str = "config") -> str:
    """Safe summary of config for debug logging (uses getattr to avoid missing-column errors)."""
    if config is None:
        return f"{label}=None"
    return (
        f"{label}=present type={type(config).__name__} "
        f"id={getattr(config, 'id', '?')} slug={getattr(config, 'slug', '?')} "
        f"is_enabled={getattr(config, 'is_enabled', '?')} "
        f"model_name={getattr(config, 'model_name', '?')} name={getattr(config, 'name', '?')}"
    )
...
async def get_agent_config(session: AsyncSession, slug: str) -> UserAgentConfig | None:
    ...
    config = result.first()
    logger.info(
        "[AGENT_INSTANCE.get_agent_config] slug=%s result=%s",
        slug,
        _config_debug_summary(config, "row"),
    )
    return config
...
async def get_user_agent_config_by_id(*, session: AsyncSession, agent_id: uuid.UUID) -> UserAgentConfig | None:
    ...
    config = await session.get(UserAgentConfig, agent_id)
    logger.info(
        "[AGENT_INSTANCE.get_agent_config_by_id] agent_id=%s result=%s",
        agent_id,
        _config_debug_summary(config, "row"),
    )
    return config
...
async def get_agent_instance(
    ...
    logger.info(
        "[AGENT_INSTANCE.get_agent_instance] slug=%s %s",
        slug,
        _config_debug_summary(config, "config"),
    )
    if config and getattr(config, "is_enabled", True):
        # Build system_prompt as before
        system_prompt = config.system_prompt or f"You are {config.name}. {config.description}"
        logger.info(
            "[AGENT_INSTANCE.get_agent_instance] slug=%s config present is_enabled=True using system_prompt len=%s",
            slug,
    ...
    logger.info(
        "[AGENT_INSTANCE.get_agent_instance] slug=%s no agent returned: config=%s is_enabled=%s",
        slug,
...
async def get_agent_instance_with_tools(
    ...
    *,
    ...
    config = await get_agent_config(session, slug)
    logger.info(
        "[AGENT_INSTANCE.get_agent_instance_with_tools] slug=%s %s",
        slug,
        _config_debug_summary(config, "config"),
    )
    if config and getattr(config, "is_enabled", True):
    ...
    logger.info(
        "[AGENT_INSTANCE.get_agent_instance_with_tools] slug=%s no agent returned: config=%s is_enabled=%s",
        slug,
        "None" if config is None else "present",
        getattr(config, "is_enabled", "?") if config is not None else "n/a",
    )
...
async def get_agent_instance_with_tools_two(
    session: AsyncSession,
    slug: str,
    *,
    user_id: uuid.UUID | None = None,
...
async def get_agent_instance_with_tools(
    ...
    For basic agents without tools, use get_agent_instance() instead.
    """
    config: Any = await get_agent_config(session, slug)
    logger.info(
        "[AGENT_INSTANCE.get_agent_instance_with_tools] slug=%s %s",
        slug,
        _config_debug_summary(config, "config"),
    )
    agent: Any = Agent(config.model_name, system_prompt=config.system_prompt)
    return agent
