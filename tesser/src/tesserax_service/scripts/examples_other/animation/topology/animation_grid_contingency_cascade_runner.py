#!/usr/bin/env python3
"""Batch runner for animation_grid_contingency_cascade.py."""
...
DEFAULT_SCENARIOS = ["line_trip", "substation_loss", "demand_spike", "multi_fault"]
DEFAULT_POLICIES = ["conservative", "balanced", "aggressive"]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Batch runner for grid contingency cascade comparison.")
    ...
    p.add_argument("--seed-start", type=int, default=97, help="Initial seed.")
    ...
    p.add_argument("--save-media", action="store_true", help="Persist per-run media files.")
    p.add_argument(
        "--output-dir",
        type=str,
        default="grid_contingency_cascade_comparison",
        help="Output directory for reports and leaderboard files.",
    )
    p.add_argument("--csv-name", type=str, default="leaderboard.csv", help="Leaderboard CSV name.")
    p.add_argument("--json-name", type=str, default="leaderboard.json", help="Leaderboard JSON name.")
...
def flatten_report(report: dict) -> dict:
    ...
    return {
        ...
        "fault_line": event.get("fault_line", ""),
        "fault_substation": event.get("fault_substation", ""),
        "service_level": float(metrics.get("service_level", 0.0)),
        "peak_overload": float(metrics.get("peak_overload", 0.0)),
        "island_count": int(metrics.get("island_count", 0)),
        "shed_load": float(metrics.get("shed_load", 0.0)),
        "recovery_time": float(metrics.get("recovery_time", 0.0)),
        "trips": int(metrics.get("trips", 0)),
        "recovers": int(metrics.get("recovers", 0)),
...
def main() -> int:
    ...
    target_script = Path(__file__).with_name("animation_grid_contingency_cascade.py")
        ...
        for policy in policies:
            for rep in range(repeats):
                ...
                print(
                    ...
                    f"peak_overload={row['peak_overload']:.3f} island_count={row['island_count']} "
                    f"shed_load={row['shed_load']:.3f} recovery_time={row['recovery_time']:.2f}s"
                )
