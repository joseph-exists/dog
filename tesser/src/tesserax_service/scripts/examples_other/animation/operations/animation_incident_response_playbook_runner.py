#!/usr/bin/env python3
"""Batch runner for animation_incident_response_playbook.py.

Runs scenario/policy matrices, collects JSON reports,
and writes ranked leaderboard outputs.
"""
...
DEFAULT_POLICIES = ["manual", "assisted", "autonomous"]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Batch runner for incident response playbook comparison.")
    ...
    p.add_argument("--seed-start", type=int, default=151, help="Initial seed.")
    p.add_argument("--seed-step", type=int, default=31, help="Seed increment per repeat.")
    ...
    p.add_argument("--save-media", action="store_true", help="Save per-run media outputs.")
    p.add_argument(
        "--output-dir",
        type=str,
        default="incident_response_playbook_comparison",
        help="Directory for output reports and leaderboard files.",
    )
...
def flatten_report(report: dict) -> dict:
    ...
    return {
        ...
        "breach_time": float(metrics.get("breach_time", 0.0)),
...
def main() -> int:
    ...
    target_script = Path(__file__).with_name("animation_incident_response_playbook.py")
        ...
        for policy in policies:
            for rep in range(repeats):
                ...
                print(
                    ...
                    f"breach_time={row['breach_time']:.2f}s mttr={row['mttr']:.2f}s"
                )
